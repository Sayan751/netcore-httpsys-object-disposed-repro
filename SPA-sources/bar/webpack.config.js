...
module.exports = ({ production }, { analyze, hot, port, host }) => {
  const baseUrl = production ? '/bar/' : '/';
  const outDir = production
    ? path.resolve(__dirname, '..', '..', 'hosting-service', 'Content', 'bar')
    ...
    plugins: [
      ...
      new CopyWebpackPlugin({
        patterns: [
          ...
          { from: 'locales', to: path.join(outDir, 'locales') },
        ]
